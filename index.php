		...
		<script type="text/javascript">
			...
			Ext.onReady(function() {
				...
				function GridHistorico() {
					...
					grid = Ext.create('Ext.grid.Panel', {
						...
						columns : [{
							...
							listeners : {
								headerclick : function( ct, column, e, t, eOpts ) {
									...
									if(VmAuxOrdenacaoHistorico =='ASC'){
									    this.up('grid').getStore().sort('idhistorico', 'DESC');
										VmAuxOrdenacaoHistorico = 'DESC';
									} else {
										this.up('grid').getStore().sort('idhistorico', 'ASC');
										VmAuxOrdenacaoHistorico = 'ASC';
									}
								}
							}
							...
							editor : {
								...
								listeners : {
									'change' : function() {
										if (this.getValue()) {
											if (tipo == 'LIGADOR' || tipo == 'ADMINISTRADOR') {
											...
											} else if (tipo == 'PSICOLOGO' || tipo == 'ADM_PSICOLOGO') {
						...
						listeners : {
							...
							'celldblclick' : function() {
								...
								grid.columns[5].field.setValue(grid.selModel.getSelection()[0].data.status);

							},
							'edit' : function(editor, e) {

								//console.log(this.getSelectionModel().selected.items[0].data);
								if (grid.columns[5].field.getValue() == null && !grid.columns[4].field.getValue() ) {
									...
									if (tipo != "ADMINISTRADOR" && tipo != "ADM_PSICOLOGO" && !verificaedicao) {
										...
										me2 = this;
										//tipo == 'ENTREVISTADOR' &&
										//  this.getSelectionModel().selected.items[0].data.status = me2.columns[5].field.value;
										// this.getSelectionModel().selected.items[0].data.comentario = me2.columns[1].field.value;

										if (grid.columns[5].field.getValue() == 'PESQUISA REALIZADA') {
											Ext.Msg.prompt('Aviso', 'Informe o Codigo do Questionario:', function(btn, text) {
												if (btn == 'ok' && /^[A-Za-z]{2}?[0-9]{4}$/.test(text)) {
													Ext.Ajax.request({
														...
														success : function(response) {
															var auxdata = {
																comentario : me2.columns[1].field.value,
																datahora : me2.columns[2].field.value,
																idhistorico : me2.columns[0].field.value,
																idpaciente : gridAtual.getSelectionModel().selected.items[0].data.idpaciente,
																idusuario : idusuario,
																mudou : me2.columns[6].field.value,
																nome : me2.columns[3].field.value,
																ressonancia : me2.columns[7].field.value,
																status : me2.columns[5].field.value,
																tentativa : me2.columns[4].field.value
															};

															//me2.getSelectionModel().selected.items[0].data
															salvaHistorio(auxdata);
											...
											if (me2.columns[5].field.value != "" && me2.columns[1].field.value != "") {
					...
					if (usuario != 'administrador') {//tipo != 'ADMINISTRADOR' && tipo != 'ADM_PSICOLOGO'
				...
				function salvaHistorio(data) {
					...
					Ext.Ajax.request({
						...
						success : function(response) {
							...
							var auxHistorico = Ext.decode(response.responseText, true);
							...
							grid.getSelectionModel().selected.items[0].data.status = me2.columns[5].field.value;
							grid.getSelectionModel().selected.items[0].data.idhistorico = auxHistorico.idhistorico;
							me2.getStore().sort('idhistorico', 'ASC');
							i.wait().hide();
				...
				function getColunas() {
					var colunas = [{
						...
						text : 'RESPONSAVEL 1',
						dataIndex : 'nome_resp1',
						hideable : true,
						flex : 1,
						hidden : true
					}, {
						text : 'RESPONSAVEL 2',
						dataIndex : 'nome_resp2',
						hideable : true,
						flex : 1,
						hidden : true
					}, {
				...
				function getFieldsHistorico() {
					return [{
							name : 'idhistorico',
							type : 'int'
						},'idpaciente', 'nome_crianca', 'comentario', 'datahora', 'nome', 'hitorico_status', 'mudou', 'ressonancia', 'tentativa'];
				}
						...
						success : function(response) {
							...
							if (data.length > 0) {
								...
								Ext.Array.forEach(data, function(paciente) {
									...
									if (paciente.status == 'MUDOU DE CIDADE') {
										dadosMudouCidade.push(montaObj(paciente));
									}
								...
								atualizagrid('localStoreMudouCidade', dadosMudouCidade);
				...
				function comboCampoConsulta(idinterno) {
					var states = Ext.create('Ext.data.Store', {
						...
						data : [{
							...
							"campo" : "idquestionario",
							"nome" : "Codigo do Questionario"
						}, {
				...
				function getLinhaConsulta(idinterno, texto_inicial, tem_botao) {
					...
					var aux3 = null;
					var aux4 = null;
					var BtnDownloadDireto = null;
					var BtnLogs = null;
					var BtnLogsHistorico = null;
					var BtnProtuario = null;
					...
					if (tem_botao) {
						...
						BtnDownloadDireto = Ext.create('Ext.button.Button', {
							name : 'dowloadDierto',
							text : 'Exibir Historico',
							handler : function() {
								...
								window.open("http://analysismg.com.br/sistema/dowloadHistorico.php?dados="+texto_consulta);
						...
						BtnLogs = Ext.create('Ext.button.Button', {
							name : 'dowloadLogs',
							text : 'Exibir Logs Pacientes',
							handler : function() {
								...
								window.open("http://analysismg.com.br/sistema/consulta_logs.php?dados="+texto_consulta);
						...
						BtnLogsHistorico = Ext.create('Ext.button.Button', {
							name : 'dowloadLogs',
							text : 'Exibir Logs Historico',
							handler : function() {
								...
								window.open("http://analysismg.com.br/sistema/consuta_logs_historico.php?dados="+texto_consulta);
						...
						BtnProtuario = Ext.create('Ext.button.Button', {
							name : 'dowProntuario',
							text : 'Exibir Prontuarios',
							handler : function() {
								...
								window.open("http://analysismg.com.br/sistema/consuta_protuarios_completos.php?dados="+texto_consulta);
					...
					return [texto_inicial_final, comboCampoConsulta(idinterno), comboComparacao(idinterno), {
						...
						xtype : 'label',
						text : ' '
					}, aux3,BtnDownloadDireto,BtnLogs, BtnLogsHistorico, BtnProtuario,aux4]
				}
				...
				function loadPage(formulario) {
					...
					viewport = Ext.create('Ext.Viewport', {
						...
						items : [{
							...
							items : [retornaGrid(ArrayStore['localStoreNIniciado'], 'N&Atilde;O INICIADO (' + dadosNContactados.length + ')'), retornaGrid(ArrayStore['localStoreEmAndamento'], 'EM ANDAMENTO (' + dadosEmAndamento.length + ')'), retornaGrid(ArrayStore['localStoreResgatado'], 'RESGATE CONFIRMADO (' + dadosResgatados.length + ')'), retornaGrid(ArrayStore['localStoreNLocalizado'], 'N&Atilde;O LOCALIZADO(' + dadosNLocalizado.length + ')'), retornaGrid(ArrayStore['localStoreRecusado'], 'RECUSADO (' + dadosRecusado.length + ')'), retornaGrid(ArrayStore['localStoreMudouCidade'], 'MUDOU DE CIDADE (' + dadosMudouCidade.length + ')'), retornaGrid(ArrayStore['localStoreNenhumFoneFunciona'], 'NENHUM TELEFONE FUNCIONA (' + dadosNenhumFoneFunciona.length + ')'), retornaGrid(ArrayStore['localStoreNaoIdentificadoEscola'], 'N&Atilde;O IDENTIFICADO ESCOLA (' + dadosNaoIdentificadoEscola.length + ')'), retornaGrid(ArrayStore['localStoreNLocalizadoFinal'], 'N&Atilde;O LOCALIZADO FINAL(' + dadosNLocalizadoFinal.length + ')'), retornaGrid(ArrayStore['localStorePESQUISA30'], 'PESQUISA 30 (' + dadosPESQUISA30.length + ')'), retornaGrid(ArrayStore['localStorePESQUISA4'], 'PESQUISA 4 (' + dadosPESQUISA4.length + ')'), retornaGrid(ArrayStore['localStoreRECUSAFINAL'], 'RECUSA NEGADO (' + dadosRECUSAFINAL.length + ')'), retornaGrid(ArrayStore['localStoreRecusa15'], 'RECUSADO A 15 DIAS (' + dadosRecusa15.length + ')'), retornaGrid(ArrayStore['localStoreAtribuidoEntrevistador'], 'ATRIBUIDO AO ENTREVISTADOR (' + dadosAtribuidoEntrevistador.length + ')'), retornaGrid(ArrayStore['localStoreAgendamentoAndamento'], 'AGENDAMENTO EM ANDAMENTO (' + dadosAgendamentoAndamento.length + ')'), retornaGrid(ArrayStore['localStoreAgendado'], 'AGENDADO (' + dadosAgendado.length + ')'), retornaGrid(ArrayStore['localStorePesquisaRealizada'], 'PESQUISA REALIZADA (' + dadosPesquisaRealizada.length + ')'), retornaGrid(ArrayStore['localStoreSituacaoEspecifica'], 'SITUACAO ESPECIFICA (' + dadosSituacaoEspecifica.length + ')'), retornaGrid(ArrayStore['localStoreResponsavelNLocalizado'], 'RESPONSAVEL NAO LOCALIZADO (' + dadosResponsavelNLocalizado.length + ')'), retornaGrid(ArrayStore['localStoreResponsavelNLocalizadoFinal'], 'RESPONSAVEL NAO LOCALIZADO FINAL(' + dadosResponsavelNLocalizadoFinal.length + ')'), retornaGrid(ArrayStore['localStoreRecusaAgendamento'], 'RECUSOU AGENDAMENTO (' + dadosRecusaAgendamento.length + ')'), retornaGrid(ArrayStore['localStoreRecusaLocal'], 'RECUSA LOCAL (' + dadosRecusaLocal.length + ')'), retornaGrid(ArrayStore['localStoreAtualizadoLocal'], 'ATUALIZADO LOCAL (' + dadosAtualizadoLocal.length + ')'), retornaGrid(ArrayStore['localStoreNaoIniciadoPsicologo'], 'NAO INICIADO PSICOLOGO (' + dadosNaoIniciadoPsicologo.length + ')'), retornaGrid(ArrayStore['localStoreAtribuidoPsicologo'], 'ATRIBUIDO AO PSICOLOGO (' + dadosAtribuidoPsicologo.length + ')'), retornaGrid(ArrayStore['localStoreAvaliacaoRealizada'], 'AVALICAO REALIZADA (' + dadosAvaliacaoRealizada.length + ')'), retornaGrid(ArrayStore['localStoreAvaliacaoAgendada'], 'AVALIACAO AGENDADA (' + dadosAvaliacaoAgendada.length + ')'), retornaGrid(ArrayStore['localStoreAvaliacaoEmAndamento'], 'AVALIACAO EM ANDAMENTO (' + dadosAvaliacaoEmAndamento.length + ')'), retornaGrid(ArrayStore['localStoreCriancaNLocalizada'], 'CRIANCA NAO LOCALIZADA (' + dadosCriancaNLocalizada.length + ')'), retornaGrid(ArrayStore['localStoreRecusaAvaliacaoAgendamento'], 'RECUSA AGENDAMENTO DE AVALIACAO (' + dadosRecusaAvaliacaoAgendamento.length + ')'), retornaGrid(ArrayStore['localStoreCriancaNLocalizadaFinal'], 'CRIANCA NAO LOCALIZADA FINAL (' + dadosCriancaNLocalizadaFinal.length + ')'), retornaGrid(ArrayStore['localStoreRecusaAvaliacaoLocal'], 'RECUSA AVALIACAO LOCAL (' + dadosRecusaAvaliacaoLocal.length + ')'), retornaGrid(ArrayStore['localStoreSituacaoEspecificaPsi'], 'SITUACAO ESPECIFICA PSICOLOGO (' + dadosSituacaoEspecificaPsi.length + ')')]
