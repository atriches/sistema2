...
$vmSql = " SELECT p.idpaciente, 
				p.sql AS dados_paciente,             
				DATE_FORMAT( p.data,  '%d/%c/%Y %H:%i:%s' ) AS data_paciente
	 		   
	FROM paciente_sql p
	INNER JOIN paciente pa on p.idpaciente = pa.idpaciente where 1=1 " . $vmcomplemento;
		...
		if ($campo == 'data_contato') {
			if ($valor != "") {
				...
				if ($aux != "") {
					...
					$vmsqllocal .= $operadorFinal . "  p.data " . $aux . "'" . $a . "-" . $m . "-" . $d . "' ";
				}
			}
		} else {
			switch ($condição) {
